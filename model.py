...
tokenizer = tf.keras.layers.TextVectorization(
    ...
    vocabulary=vocab#vocabulary 可选的。字符串数组或文本文件的字符串路径。如果传递一个数组，可以传递一个元组、列表、一维 numpy 数组或包含字符串词汇项的一维张量。如果传递文件路径，则该文件应包含词汇表中的每个术语一行。如果设置了此参数，则无需adapt() 图层。
)
#可以返回indices编码，onehot编码，multihot编码；逆转编码，可以根据indices返回对应的原始词。
...
#设置不同的层，继承tf.keras.layers.Layer
...
def generate_caption(img, caption_model, add_noise=False):
    ...
    img = tf.expand_dims(img, axis=0)#增加维度
